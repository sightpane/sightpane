...
- **SQL Rows lifecycle and error checks.** Every `for rows.Next()` loop must check `rows.Err()` immediately after completion (`if err := rows.Err(); err != nil`). Scan errors must never be swallowed (`err == nil` is a finding). `defer rows.Close()` must never be placed directly inside loops — use scoped closures or explicit closing.
