...
- **Commit on issue/task completion**: Whenever an issue, bug fix, or task is finished, run the `/code-auditor` skill on modified files, resolve all findings, verify static checks and tests (`go vet`, `go test -race`), then create a descriptive conventional commit explaining what was changed and why. Do not run `git push` unless explicitly asked.

**Commits and PRs.** Commit and push your own branch; never commit to `main` or rewrite history you didn't create. One commit per sub-task, each standing on its own and leaving the tree working; if the message needs an "and", it's two commits, but ten one-liners are as unreviewable as one 2,000-line dump. Keep each commit coherent across the stack (endpoint with its schema and tests; consuming UI separately), separate refactors from behaviour changes, and order commits so each builds on the last. Conventional prefixes (`feat:`, `fix:`, `refactor:`, `test:`, `docs:`), scoped where it helps (`fix(admin): …`). Open PRs as drafts (`--draft`) while work is ongoing, and mark them ready (`gh pr ready`) only when 100% complete: tests written first and green, the revert-and-watch-red check done, lint clean, the docs your change invalidates updated, and a PR body (`pr-writer`) that says what you verified and what you couldn't.
